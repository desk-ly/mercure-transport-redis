...
Anything with write access to `<stream>:pubsub` can inject updates into the hub, which are then
dispatched to subscribers without further authorization. Run Redis with AUTH/ACLs and TLS, and do not
share the instance with untrusted workloads. Updates are stored in the stream unencrypted.
...
Tests run against [miniredis](https://github.com/alicebob/miniredis), so no Redis instance is needed.
